...
public class BlankFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        // 만일 fragment_blank.xml 문서에 myTextView 라는 아이디를 가지고 있는 TextView 의 참조 값이 필요하다면?
        TextView a = binding.myTextView; // 이렇게 참조하면 된다.

        return view;
        // return inflater.inflate(R.layout.fragment_blank, container, false);
    }
    // 프래그먼트가 레이아웃으로 가지고 있는 뷰가 파괴될때 호출되는 메소드
    @Override
    public void onDestroyView() {
        super.onDestroyView();
        // FragmentBlankBinding 객체의 참조를 해제해서 메모리를 효율적으로 사용하도록 한다.
        binding=null;
        // 프래그먼트 보다 뷰가 더 오래 살아남기때문에 프래그먼트는 사용되지 않는데
        // 뷰만 메모리에 남아 있는 것을 방지하는 효과가 된다.
    }
}