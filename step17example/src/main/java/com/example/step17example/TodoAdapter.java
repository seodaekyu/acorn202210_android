...
public class TodoAdapter extends BaseAdapter {
    ...
    public TodoAdapter(Context context, int layoutRes, List<TodoDto> list) {
        this.context = context; // 액티비티의 참조 값
    ...
    // position 에 해당하는 모델을 리턴 (할일)
    @Override
    public Object getItem(int position) { // Object type 으로 리턴하기때문에 자식 type 도 리턴 가능하다.
        return list.get(position);
    }
    // position 에 해당하는 아이템의 아이디(primary key) (할일의 번호)
    @Override
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        // View 의 모델이 가지고 있는 데이터를 출력하고
        TodoDto dto = list.get(position);

        // ListView_cell.xml 안에 있는 TextView 의 참조 값 얻어와서 문자열 출력하기
