...
class TodoDao(var helper:DBHelper) {
    ...
    fun insert(data:Todo){
        // in Java => SQLiteDataBase helper.getWritableDatabase()
        val db = helper.writableDatabase

        // ? 에 바인딩할 인자를 Any type 배열로 얻어내기
        // in Java => Object[] args = { data.getContent() }
        val args = arrayOf<Any>(data.content)

        // 실행할 sql 문
        // SQLiteDB => datetime('now', 'localtime') in Oracle => SYSDATE
    ...
    fun getList():List<Todo>{
        // 수정 가능한 Todo type 을 담을 수 있는 List
        ...
        /*
           ...
           month : %m
           date : %d
           week of day 0 1 2 3 4 5 6 : %w   / 일 월 화 수 목 금 토
           hour : %H
           minute : %M
        ...
        // query 문을 수행하고 결과를 Cursor 객체로 얻어내기 (selection 인자는 없다. ex_WHERE num)
