...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        requestBtn.setOnClickListener(v->{
            ...
            // http 요청은 시간이 오래 걸릴 수 있는 불확실한 작업이다. => 비동기 task 에서 작업해야 한다.
    ...
    class RequestTask extends AsyncTask<String, Void, String>{
        ...
        protected String doInBackground(String... strings) {
            ...
            try{
                ...
                // 정상적으로 연결이 되었다면?
                if(conn != null) {
                    conn.setConnectTimeout(20000); // 응답을 기다리는 최대 대기시간
                    conn.setRequestMethod("GET"); // 요청 메소드 설정 Default 설정 (Default 는 GET 방식)
                    conn.setUseCaches(false); // 캐쉬 사용 여부
                    // 응답 코드를 읽어온다.
                    int responseCode = conn.getResponseCode();
                    if(responseCode == HttpsURLConnection.HTTP_OK) { // 정상 응답이라면 200 번
                        // 문자열을 읽어들일 수 있는 객체의 참조 값 얻어오기
        ...
        protected void onPostExecute(String s) {
            ...
            // 여기는 UI Thread 이고 s 에는 서버가 응답한 문자열이 들어있다.
            editText.setText(s);
        }
    }
